...
static void action_rename_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    ...
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        if(R_SUCCEEDED(res)) {
            ...
            prompt_display_notify("成功", "名前を変更しました。", COLOR_TEXT, NULL, NULL, NULL);
        } else {
            error_display_res(NULL, NULL, res, "名前の変更に失敗しました。");
        }
    }
...
void action_rename(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "名前変更データの割り当てに失敗しました。");
    ...
    kbd_display("新しい名前を入力してください", ((file_info*) selected->data)->name, SWKBD_TYPE_NORMAL, 0, SWKBD_NOTEMPTY_NOTBLANK, FILE_NAME_MAX, data, action_rename_onresponse);
}