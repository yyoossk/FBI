...
static void action_install_cias_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("CIAのインストール", "Bボタンを押してキャンセル。", true, data, action_install_cias_update, action_install_cias_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "CIAのインストールを開始できませんでした。");
...
static void action_install_cias_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            error_display_res(NULL, NULL, loadingData->popData.result, "CIAリストへの入力に失敗しました。");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "CIAリストの取得中...");
}
...
static void action_install_cias_internal(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData, const char* message, bool delete) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "インストールCIAデータの割り当てに失敗しました");
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "CIAリストの作成を開始できませんでした。");
    ...
    info_display("読み込み中", "Bボタンを押してキャンセル。", false, loadingData, action_install_cias_loading_update, action_install_cias_loading_draw_top);
}

void action_install_cia(linked_list* items, list_item* selected) {
    action_install_cias_internal(items, selected, NULL, NULL, "選択したCIAをインストールしますか?", false);
}

void action_install_cia_delete(linked_list* items, list_item* selected) {
    action_install_cias_internal(items, selected, NULL, NULL, "選択したCIAをインストールおよび削除しますか?", true);
}
...
void action_install_cias(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData) {
    action_install_cias_internal(items, selected, filter, filterData, "現在のディレクトリにすべてのCIAをインストールしますか?", false);
}
...
void action_install_cias_delete(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData) {
    action_install_cias_internal(items, selected, filter, filterData, "現在のディレクトリにあるすべてのCIAをインストールおよび削除します?", true);
}