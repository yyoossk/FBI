...
static void action_paste_contents_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(pasteData->pasteInfo.finished) {
        ...
        if(R_SUCCEEDED(pasteData->pasteInfo.result)) {
            prompt_display_notify("成功", "貼り付けた内容。", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_paste_contents_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("内容の貼り付け", "Bボタンを押してキャンセル。", true, data, action_paste_contents_update, action_paste_contents_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "貼り付け操作の開始に失敗しました。");
...
static void action_paste_contents_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            error_display_res(NULL, NULL, loadingData->popData.result, "クリップボードのコンテンツリストに入力できませんでした。");
...
void action_paste_contents(linked_list* items, list_item* selected) {
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "クリップボードコンテンツリストの作成を開始できませんでした。");
    ...
    info_display("読み込み中", "Bボタンを押してキャンセル。", false, loadingData, action_paste_contents_loading_update, action_paste_contents_loading_draw_top);
}