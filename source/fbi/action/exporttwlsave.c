...
static bool action_export_twl_save_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(((export_twl_save_data*) data)->title, task_draw_title_info, res, "セーブのエクスポートに失敗しました。");
    return true;
}
...
static void action_export_twl_save_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(exportData->exportInfo.finished) {
        ...
        if(R_SUCCEEDED(exportData->exportInfo.result)) {
            prompt_display_notify("成功", "エクスポートして保存します。", COLOR_TEXT, exportData->title, task_draw_title_info, NULL);
        }
...
static void action_export_twl_save_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("セーブのエクスポート", "Bボタンを押してキャンセル。", true, data, action_export_twl_save_update, action_export_twl_save_draw_top);
        } else {
            error_display_res(exportData->title, task_draw_title_info, res, "セーブのエクスポートの開始に失敗しました。");
            free(data);
        }
    } else {
        free(data);
...
void action_export_twl_save(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "エクスポートTWLセーブデータの割り振りに失敗しました。");
