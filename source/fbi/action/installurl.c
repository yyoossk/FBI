...
static void action_install_url_confirm_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("URLからのインストール", "Bボタンを押してキャンセル。", true, data, action_install_url_install_update, action_install_url_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "インストールを開始できませんでした。");
...
void action_install_url(const char* confirmMessage, const char* urls, const char* paths, void* userData,
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "URLインストールデータの割り当てに失敗しました");
