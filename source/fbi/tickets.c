...
static list_item delete_ticket = {"チケットを削除", COLOR_TEXT, action_delete_ticket};
static list_item delete_unused_tickets = {"未使用のチケットを削除する", COLOR_TEXT, action_delete_tickets_unused};
...
static void tickets_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "チケットアクションデータの割り当てに失敗しました。");
...
static void tickets_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "チケットリストの作成を開始できませんでした。");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "チケットリストの入力に失敗しました。");
...
void tickets_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "チケットデータの割り当てに失敗しました。");
    ...
    list_display("チケット", "A: 選択, B: 戻る, X: リフレッシュ", data, tickets_update, tickets_draw_top);
}