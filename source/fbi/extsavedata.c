...
static list_item browse_user_save_data = {"ユーザーセーブデータを観覧", COLOR_TEXT, action_browse_user_ext_save_data};
static list_item browse_spotpass_save_data = {"スポットパスセーブデータを観覧", COLOR_TEXT, action_browse_boss_ext_save_data};
static list_item delete_save_data = {"セーブデータを削除", COLOR_TEXT, action_delete_ext_save_data};
...
static void extsavedata_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "extセーブデータのアクションデータの割り当てに失敗しました。");
    ...
    list_display("Extセーブデータアクション", "A: 選択, B: 戻る", data, extsavedata_action_update, extsavedata_action_draw_top);
}
...
static void extsavedata_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        extsavedata_options_add_entry(items, "SDを表示", &listData->showSD);
        extsavedata_options_add_entry(items, "NANDを表示", &listData->showNAND);
        extsavedata_options_add_entry(items, "IDで並べ替え", &listData->sortById);
        extsavedata_options_add_entry(items, "名前で並べ替え", &listData->sortByName);
    }
}

static void extsavedata_options_open(extsavedata_data* data) {
    list_display("オプション", "A: トグル, B: 戻る", data, extsavedata_options_update, NULL);
}
...
static void extsavedata_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "extセーブデータリストの作成を開始できませんでした。");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "extセーブデータリストへの入力に失敗しました。");
...
void extsavedata_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "extセーブデータデータの割り当てに失敗しました。");
    ...
    list_display("Extセーブデータ", "A: 選択, B: 戻る, X: リフレッシュ, Select: オプション", data, extsavedata_update, extsavedata_draw_top);
}